...
#my %ADM_freq; $ADM_freq{std}=[split //,"10000000001000000000100000000010000000001000000000100000000010000000001000000000100000000010000000001000000000100000000010000000001000000000"];
#$ADM_freq{std1}=[split //,"10000000001000000000100000000010000000001000000000100000000010000000001000000000100000000010000000001000000000100000000010000000001000000000"];
#my  ($fftPower,$peakPercent,$phaseAngle,$peakArea)=ftADM (\%ADM_freq);
#exit;
...
#	my @test=(split //,"000010111010000000100000001001110000000110000000011000000001100000101111000000011111000001100000000010100000001100000010110000000001100000000011100");
#    $ADM_freq{StdfromCrystalstructure}=\@test;
    ...
    foreach my $keynt (keys(%ADM_freq))
    {
        ...
        my $frontSpl=0; my $endSpl = -(0 + $splice_l);
		if($endSpl==0) {$endSpl=1000;} #adjust the input length to be the same for multi nt
		...
		my $phaseCorrect = 28;# bp according to the head of lig # should be 24 for Truseq ligand
		
		$phaseCorrect = 0 if $keynt =~ m/std/i; # no phase corr for test ligand
		
        #print Dumper(@spliced);die;
        ##########################
        # substract mean freq before calc
        my $meanOfKey= &mean(@spliced); # used as normalization factor
        for (my $i = 0; $i < scalar(@spliced); ++$i) # only 
				{
        ...
        my $phaseAngle_h196_corr=$phaseAng_h196-2*($phaseCorrect/10.204); # correct for the phase of splicing at the start
