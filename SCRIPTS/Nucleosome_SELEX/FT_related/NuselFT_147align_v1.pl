...
#*** INIT ***
    ...
    my $usage = "\n\$$this_programme -f '*.txt' -b '*.txt' -k 2 -t 2 -st 10 --samebk
  ...
  ## specific ##
   ...
   ";
	##### options
	GetOptions ('f=s' => \my $fileGlob, 'b=s' => \my $bkfileGlob,'o=s' => \my $outputPath, 'h' => \my $helpInfo,'bkADM' => \my $bkisADM, 'samebk' => \my $samebk, 'l=i' => \my $lines, 'k=i' => \my $k,'t=i' => \our $threads, 'st=i' => \my $sub_threads, ); # i:int   f:float   s:string
...
chdir $outputPath;
system ('Rscript /wrk/data/fangjie/lib/SCRIPTS/Nucleosome_SELEX/FT_related/R_NuselFT_align.R');
exit;
...
sub singleProc  # each file
{
	...
	if ($bkfile)
	{
		...
		die "liglength in bk file not same as sig" if @{$sum->{(keys $sum)[0]}} != @{$sumbk->{(keys $sum)[0]}} ; #check if $liglengthbk != $liglength
