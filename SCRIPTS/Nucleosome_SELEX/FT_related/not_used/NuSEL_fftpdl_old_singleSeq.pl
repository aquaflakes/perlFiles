...
{   #loop through all files
		...
		my %peakHeightNorm;
		my @keys= qw (A T C G AA AT AC AG CA CT CC CG GA GT GC GG TA TT TC TG); #qw(AA GG TA); #
		...
		while (my $seq = <$seq>)
		{
			...
			my ($peakHeightHR,$ntCountHR,$ntCntSqr) = &fft_win::ftSRfreqPDL($Nufreq);
			#unless (($peakHeightHR->{$_})**2/$ntCountHR->{$_}) {	print "peakHeightHR".$peakHeightHR->{$_}."ntCountHR".$ntCountHR->{$_}."\n";	}
			
			for (@keys) { if ($ntCountHR->{$_}) {$peakHeightNorm{$_}+=($peakHeightHR->{$_})**2/($ntCountHR->{$_}); $peakHeight{$_}+=$peakHeightHR->{$_}; $ntCount{$_}+=$ntCountHR->{$_};}     }
			$lineCnt=$.;$row_param--; $row_param||last;
			
		}
		close $seq;
		
		my %normalizedPeak= map {$_, $peakHeightNorm{$_}/$lineCnt."\t".$peakHeight{$_}."\t"."$ntCount{$_}"} @keys;
		my $addedCap;
		if ($bkcrr)
		{
				my @cyc0_bkfiles = glob("/wrk/data/fangjie/seqFiles/cyc0/analysis/NuSEL_fftpdl_singleSeq.pl._/*.txt");
				...
				%normalizedPeak= map{$_,($peakHeightNorm{$_}/$lineCnt-$nt_freq_bk->{$_}->[0])."\t".$normalizedPeak{$_}} @keys;
				$addedCap=["key","Peak10.2crr","normalizedPeak10,2H","peakHeight","ntAbsCounts"];
				}
				#print Dumper ($nt_freq_bk);die; 1000000
