...
use File::Basename;
use perllib::Me::parallel_rc;
use Data::Dumper;
use List::Util qw(sum);
use perllib::Me::FT::fft_win_splice;
...
{   #loop through all files
    ...
    &parallel_rc::calc($filename, $liglength, $row_param, $cores, $tmpFile_path, $isFastq,      $reads_anal_methodRef, $tmpHash_output_methodRef,      $tmpreads_output_methodRef, $filterHashRef ); #process for the files <<<<<<<<<<<<<<<
	...
	my ($fftPower,$peakPercent,$phaseAngle,$peakArea) = &fft_win_splice::ftADM($Summed_ADM_freq);
...
sub init_cntHash # add keys and 0 s for each key 
{
    my ($href, $liglength, $baseAref, $max_nt_length)= @_;
	my $curr_nt; my @inibases=@$baseAref;
	for (1..$max_nt_length-1) {@$baseAref= map {$curr_nt=$_, map {$curr_nt.$_} @inibases} @$baseAref};
    map {$href->{$_}=[split //,0 x ($liglength-length($_)+1)]} @$baseAref;
}
