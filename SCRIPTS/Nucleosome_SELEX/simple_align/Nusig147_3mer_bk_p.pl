#!/usr/bin/perl -w
#20150903 edited from Nusig147_freq
...
my $reads_anal_methodRef = \&ADM::addSR_simple_align3mer; # process reads and store result in hash tmp, return a hash, write after all reads finished
my $tmpHash_output_methodRef = \&ADM::wtADM; # write hash in the process of each core into a tmp file
my $Summed_hash_output_methodRef = \&ADM::WriteSummedADM3mer;
...
if ($file_param eq "all")
	{@files = <*147*.txt>; } #all files in the folder into the array
	else
...
{   #loop through all files
	...
	my $Summed_ADM_bf_bkCorr= &ADM::rdADM_asIs_3mer($Summed_ADM_path); system ("rm $Summed_ADM_path");
	my $Summed_ADM_bf_bkCorr_lineCnt = &ADM::linecntFromADM($Summed_ADM_bf_bkCorr);#use the sum of the 5th position as the line count for 3mer only
				#print scalar(keys($Summed_ADM_bf_bkCorr)) ;die;	
	
		#---read in background ADM----
		"#########"=~ m/(.*)/; #reset $1
	my @cyc0_bkfiles;
	unless ($currpath=~m/cyc0/||$filename=~m/_0.txt/) {@cyc0_bkfiles= glob("/wrk/data/fangjie/seqFiles/cyc0/analysis/$this_programme/ADM/*")}; # no bk crr for cyc0 files
	...
	if ($target_bkfile) #when bk exists correct for bk
	{
		...
		my $bck_ADM_c0= &ADM::rdADM_asIs_3mer($target_bkfile);
		$bck_ADM_c0= &ADM::trim_multint($bck_ADM_c0,$liglength);	
		$Summed_ADM_bf_bkCorr= &ADM::trim_multint($Summed_ADM_bf_bkCorr,$liglength);
		
		for (split("","ATCG")){delete $Summed_ADM_bf_bkCorr->{$_};}	#otherwise it will try to count the bk
		my $bck_ADM_c0_lineCnt =&ADM::linecntFromADM($bck_ADM_c0);
	...
	unless ($currpath=~m/cyc0/||$filename=~m/_0.txt/) {$Summed_ADM_freq = &ADM::cntToFreqv1($Summed_ADM_af_bkCorr); }# convert counts to frequency
	...
	$Summed_ADM_freq=&ADM::trim_multint($Summed_ADM_freq,$liglength);
	for (split("","ATCG")){delete $Summed_ADM_freq->{$_};}

    my $Summed_ADM_freq_nomalized=$Summed_ADM_freq;
	unless ($currpath=~m/cyc0/||$filename=~m/_0.txt/) {$Summed_ADM_freq_nomalized=&normalize($Summed_ADM_freq);} # devided by average and minus average
	$Summed_ADM_freq->{pos}= \my @emptyArr; push $Summed_ADM_freq->{pos},(1..scalar(@{$Summed_ADM_freq->{AAA}})); # position info for the firstline of output		
	&ADM::wtADM_asIs3mer_capLine($Summed_ADM_freq,$filename_final_anal,$final_output_path."/ADM","pos","--prefill"); #fill diNt with 0 in the end bit 
...
########## plot ##########
#my $ADM_output_path=$final_output_path;
#chdir $ADM_output_path;
#system ('R --no-save </wrk/data/fangjie/lib/SCRIPTS/Nucleosome_SELEX/FT_related/R_NuselFT_align.R');
