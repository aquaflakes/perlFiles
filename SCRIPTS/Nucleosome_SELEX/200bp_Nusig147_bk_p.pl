...
if ($file_param eq "all")
	{@files = <*_200*>; } #all files in the folder into the array
	else
...
{   #loop through all files
	...
	my $Summed_ADM_bf_bkCorr= &ADM::rdADMv200_notlg($Summed_ADM_path);
	my $Summed_ADM_bf_bkCorr_lineCnt = $Summed_ADM_bf_bkCorr->{"A"}->[4]+$Summed_ADM_bf_bkCorr->{"T"}->[4]+
		...
		"#########"=~ m/(.*)/; #reset $1
	my @cyc0_bkfiles = </wrk/data/fangjie/seqFiles/cyc0/analysis/200bp_Nusig147_bk_p.pl/ADM/*>;
	my $target_bkfile; $filename_no_ext =~ m/_(200[0-9]*[A-Z]*)[0-9A-Z]*_/;  # m/_(147[0-9A-Z]*)_/;
	my $match_pattern= $1;
	...
	if ($target_bkfile) #when bk exists correct for bk
	{
		...
		my $bck_ADM_c0= &ADM::rdADMv200_notlg($target_bkfile);
	...
	&ADM::wtADM($Summed_ADM_af_bkCorr,$filename_final_anal,$liglength,$final_output_path);
	#-----------bk corr end--------	
