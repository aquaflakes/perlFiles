#!/usr/bin/perl -w
# 2015-11-09  compare freq difference of k-mers between files
use warnings;
use strict;
use Getopt::Long;
use File::Basename;
...
formula: log_fold_change= log_base2 [(freq_sigFile - freq_refFile) / (freq_refFile + freq_accords_to_pseudo_cnt_for_refFile*multiply_factor) + 1]
NB!! *should be identical numbers of sigFiles and refFiles. 1st sig accords to 1st ref, 2st accords to 2st ...
  ...
  ## general ##
   -s 'str' input signal files, can be './xxx/{file1.txt, file2.txt, ...}' or './xxx/file{1..4}.txt' or 'xxx/*.txt' or 'xxx/*'
            NB!!    quotation '' for -f is important otherwise have to escape {, }, * ... with \\
...
my @files_sig = `ls $fileGlob_sig`;chomp @files_sig;
my @files_ref = `ls $fileGlob_ref`;chomp @files_ref;
if ($#files_sig!=$#files_ref) { print "\nusage: $usage\n";exit;} # ensure identical number of sig and ref files
my @params= map [$files_sig[$_],$files_ref[$_]], 0..$#files_sig; # pack parameters into units for each thread
...
{
  my $param_unit = shift;
  my $file_sig=$param_unit->[0]; my $file_ref=$param_unit->[1];
  print "\nsig $file_sig is compared to ref $file_ref with pseudo count $pseudoCnt\n";
  my %kmerFreq;
  ...
  foreach (keys %kmerFreq)
  {
    ...
    my $multiply_factor= (($$sigCnt<10)&&($$refCnt<10))? 5:1;# increase pseudo by 5 when both counts are low
    #print $multiply_factor."sig".$$sigCnt."ref".$$refCnt."\n";
    $kmerFreq{$_}->{logFoldChn}= log(($$sigFreq-$$refFreq)/($$refFreq+$pseudoFreq*$multiply_factor)+1)/log(2);
  }
