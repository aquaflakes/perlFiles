...
my $usage = "\n\$$this_programme -f '*.txt' -o kmer_cnt -k 6 --m --u
  ...
  ## specific ## 
   ...
   --u       remove duplicate ligand
   ";
	##### options
	GetOptions ('f=s' => \my $fileGlob,'o=s' => \my $outputPath, 'h' => \my $helpInfo,'u' => \my $rmdup, 'k=i' => \my $k_length, 'w=i' => \my $win_size,'m' => \my $multiCntperSeq, 'bin' => \my $bin, 'rc' => \my $rc,'t=i' => \my $threads); # i:int   f:float   s:string
    # initialize
...
sub SeqAnal {
  ...
  foreach(@allreads) { countKmer(\$_,$kmer_Href,$totalCntRef); } # count kmer of all seq
  foreach (keys $kmer_Href) { delete $kmer_Href->{$_} if ($_=~ m/N/);} # throw away keys with N
  
  if ($rmdup) {  
  &seqFile::rmdup_3point(\@allreads,$win_size);
  foreach(@allreads) { countKmer(\$_,$kmer_rmdup_Href,$totalCnt_rmdup_Ref); } # count kmer in deduplicated seq
  foreach (keys $kmer_rmdup_Href) { delete $kmer_rmdup_Href->{$_} if ($_=~ m/N/);} # throw away keys with N
  }
}

sub countKmer {
  my ($seq_p, $kmer_Href, $totalCntRef)=@_;
  my $k = $k_length;
  my %beenThere;
