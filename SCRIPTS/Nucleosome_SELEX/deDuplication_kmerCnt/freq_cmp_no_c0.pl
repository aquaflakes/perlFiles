...
formula:   logFoldChnSig= (sigFreq - refFreq) / (refFreq + pseudoFreq) + 1;
           logFoldChn= logFoldChnSig / log(2);
...
my @files_sig = glob($fileGlob_sig);chomp @files_sig; 
my @files_ref = glob($fileGlob_ref);chomp @files_ref; 
...
sub singleThread    #!!!!!!!  the thing done by each thread, to be modified!!!!!!!
{
  ...
  foreach (keys %kmerFreq)
  {
    ...
    $kmerFreq{$_}->{logFoldChnSig}= ($$sigFreq-$$refFreq)/($$refFreq+$pseudoFreq*$multiply_factor)+1;
    $kmerFreq{$_}->{logFoldChn}= log($kmerFreq{$_}->{logFoldChnSig})/log(2);
  }
