#!/usr/bin/perl -w
# 2015-05-20
# calc info content per each position against same cyc0, '-A1-' like string should be in file name, '-A01-' OK
...
my @history=system("history");
chomp @history; use Data::Dumper;
print Dumper(@history); die;


my $this_programme = basename($0);
my $cmdInput=$this_programme." ".join(" ", @ARGV);
my $usage = "\n\$$this_programme -f '*.txt' -b '*.txt' -g '*.txt'.txt -o infoCnt_bkNum20k_p1/MeOri_c1 -k 6 --m -u -t 30 -bkNum 20000 -p 1 -l 100000 -app info_c1
  ...
  ## specific ##
   ...
   ";
	##### options
	GetOptions ('f=s' => \my $fileGlob,'b=s' => \my $bkfile, 'g=s' => \my $guidefile, 'phpdir=s' => \my $phpdir, 'o=s' => \my $outputPath, 'h' => \my $helpInfo,'u' => \my $rmdup, 'k=i' => \my $k_length, 'w=i' => \my $win_size,'m' => \my $multiCntperSeq,
