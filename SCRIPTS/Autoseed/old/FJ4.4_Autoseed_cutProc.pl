...
my $usage = "\n\$$this_programme [JustReads file | folder]\n
	...
	Example: $this_programme -f 'file' -g 'FACcut_t' (-b 0) (--jr -l 51) --copy (--test first and check guidefile) --phponly -sep III
...
########## Info from cmd line #################
	...
	GetOptions ('l=i' => \my $lengthDef, 'guide' => \my $guideFile, 'phponly' => \my $phponly,'test' => \my $isTest, 'copy' => \my $toCopy, 'c=i' => \my $cores, 'g=s' => \my $guideName_add,
				'jr' => \my $isJustReadDir, 'b=s' => \my $bk_cyc, 'sep=s' => \my $sep, 'o=s' => \my $outputPath, 'overwrite' => \my $overwrite, 'f=s' => \my $files);#, 'c=i' => \$cores, 'n=s' => \$appended_foldername, 'p=f' => \$p_value ); # i:int   f:float   s:string
