...
my $usage = "\n\$$this_programme [JustReads file | folder]\n
	...
	Example: $this_programme -f 'file' -g 'FACcut_t' (-b 0) (--jr -l 51) --copy (--test first and check guidefile) --phponly -sep III -c 20 -o outDir 
	...
	***options***
	...
	-sep    seperation character between fields
	-std	using old SELEX-file naming (not working)
	-overwrite	  overwrite
	-infoStart    string that precede the sample info for parsing sample info
...
########## Info from cmd line #################
	...
	GetOptions ('l=i' => \my $lengthDef, 'guide' => \my $guideFile, 'phponly' => \my $phponly,'test' => \my $isTest, 'copy' => \my $toCopy, 'c=i' => \my $cores, 'g=s' => \my $guideName_add,
				'jr' => \my $isJustReadDir, 'b=s' => \my $bk_cyc, 'sep=s' => \my $sep, 'o=s' => \my $outputPath, 'overwrite' => \my $overwrite, 'infoStart=s' => \my $infoStart, 'f=s' => \my $files, 'std'=> \my $std);#, 'c=i' => \$cores, 'n=s' => \$appended_foldername, 'p=f' => \$p_value ); # i:int   f:float   s:string
	print($usage) && exit unless $sep;
	##### remaining param
	...
	if ($guideName_add)
	{
		...
		system("cp -a $wkPATH/DefaultFilesforProc/guidefiles/template/blank_guide_1.txt $outputPath");
		$guideFileName="$outputPath/$guideName_add"."_Fangj".strftime('%Y%m',localtime).".txt";
		system("mv $outputPath/blank_guide_1.txt $guideFileName") unless $phponly;
...
{   
	...
	if ($division==1) { $division=2;} # at least 1 division is required
	...
	$JustReadsFile =~ s/\Q$sep\E/S40N\Q$sep\E/; #!!!!!S40N is important for Autoseed to run
	my $outFilename;
	if(!$std)
	{
		$JustReadsFile =~ m{\/([^/_]*\Q$sep\Ec.)};
		$outFilename = $1;
	} else
	{
		$JustReadsFile =~ m{\/([^/]*\Q$sep\E.)};
		$outFilename = $1;
	} #m{\/([^/_]*)[^/]*\.\w+$}; 
	...
	my $batch;
	if(!$std) {
				$outFilename =~ m{(?P<barcode>.*)(?P<batch>\Q$sep\E.*\Q$sep\Ec)(?P<cyc>.)$};
				$cyc = $+{cyc};
				$bar_in_guide=($+{barcode});
				$batch = $+{batch};	   
			   } 
	else  {
			$outFilename =~ m{(?P<barcode>.*)(?P<batch>\Q$sep\E.*\Q$sep\E)(?P<cyc>.)$};
						$cyc = $+{cyc};
				$bar_in_guide=($+{barcode});
				$batch = $+{batch};	
			}

	$bar_in_guide=~ tr/-/./; $bar_in_guide =~ s/\Q$sep\E/./g;
	$batch=~ tr/-/./; $batch=~ s/\Q$sep\E/./g;
	$cyc=~ tr/-/./; $cyc=~ s/\Q$sep\E/./g;
	
	$outFilename =~ tr/-/./; $outFilename =~ s/\Q$sep\E/./g;
	...
	if ($guideName_add && (defined($cyc)) && !($cyc==0)) # not writing guide file when cyc0
	{
		...
		my $Info;
		#if ($std) {$JustReadsFile =~ m/-([^-]*)\Q$sep\Ec/;$Info=$1;} else {$JustReadsFile =~ m/-([^-]*)\Q$sep\E/; $Info=$1; } 
		if ($std) {$JustReadsFile =~ m/-([^-]*)\Q$sep\Ec/;$Info=$1;} 
		elsif ($infoStart) { $JustReadsFile =~ m/\Q$infoStart\E-{0,1}(.*?)-{0,1}\Q$sep\E/; $Info=$1;}
		else {$JustReadsFile =~ m/-([A-Za-z]+?[0-9-]*?)-{0,1}\Q$sep\E/; $Info=$1; }

		$JustReadsFile =~ m/-([A-P]{1}\d{1,2})-/; my $well=$1;   
		$well=~ s/([A-P])0(\d)/$1$2/;  # to change A01 into A1 for SELEX
		
		open(my $guide_fh, ">>", $guideFileName) or die "cannot open guidefile to append";
			print $guide_fh "NU\t"."Full\t"."$bar_in_guide\t".$batch."\t"."AA\t"."1\t".$cyc."\t$well\t$Info\t-;\n";
		close $guide_fh;
	}
...
if ($guideName_add)
{
	...
	system ("sort -r -k 4 $guideFileName| awk -F\"\\t\" '!_[\$4]++'"." |tac >$guideFileName"."1"); # awk extract only uniq records according to filed 3, 3b0
	my $guidefile1="$guideFileName"."1"; my $tailline=`tail -n 1 $guidefile1`; #print "##### tail: \n $tailline \n"; die;
	if ($tailline=~ /^GROUP/) {
		system ("tac $guidefile1 > $guideFileName"); # sometimes the order can be reversed
	}else
	{
...
if ($toCopy)
{
	...
	system("cp --remove-destination $outputPath/*.seq /var/www/kaz/data/sequence");
	if ($guideName_add)
	{
		print "\n\n"; system("ls $outputPath/*Fangj*.txt"); print "copying guide\n";
		system("cp -a $outputPath/*Fangj*.txt /var/www/kaz/guide");
		...
		# run all cycs
		system("nohup php asks_gen.php $guideFileNameOnly all order=0123 10$ow thread=10 >$outputPath/nohup_ask_gen.txt&"); # "&" to run at background
		
		# run also the bk cyc if defined b0
		if (defined($bk_cyc))
		{	# cyc like 4b0 will not run unless delete "all"
			system("nohup php asks_gen.php $guideFileNameOnly order=0123 10$ow thread=10 >$outputPath/nohup_ask_gen_b0.txt&"); #ksow
		}
		print("nohup log to $outputPath/nohup_ask_gen.txt\n");
		#
		# nohup php asks_gen.php FJ_HT_lig147_all_success all order=0123 10 overwrite thread=10 >~/phpout_ht.txt&
		# nohup php asks_gen.php FJ_NCAP_lig147_all_success all order=0123 10 overwrite thread=10 >~/phpout_ncap.txt&
