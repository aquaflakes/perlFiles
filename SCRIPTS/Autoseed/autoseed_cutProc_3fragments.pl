...
########## Info from cmd line #################
	...
	GetOptions ('l=i' => \my $lengthDef, 'guide' => \my $guideFile, 'phponly' => \my $phponly,'test' => \my $isTest, 'copy' => \my $toCopy, 'c=i' => \my $cores, 'g=s' => \my $guideName_add,
				'jr' => \my $isJustReadDir, 'b=s' => \my $bk_cyc, 'sep=s' => \my $sep, 'o=s' => \my $outputPath, 'overwrite' => \my $overwrite, 'f=s' => \my $files, 'std'=> \my $std);#, 'c=i' => \$cores, 'n=s' => \$appended_foldername, 'p=f' => \$p_value ); # i:int   f:float   s:string
...
{   
	...
	if ($guideName_add && (defined($cyc)) && !($cyc==0)) # not writing guide file when cyc0
	{
		...
		if ($std) {$JustReadsFile =~ m/-([^-]*)\Q$sep\Ec/;$Info=$1;} else {$JustReadsFile =~ m/-[A-P]{1}\d{1,2}-(.{1,40})\Q$sep\E/; $Info=$1; } 
